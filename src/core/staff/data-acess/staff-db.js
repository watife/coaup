export default function makeStaffDb ({ makeDb, StaffModel, isValid }) {
  ...
  async function findOne(id) {
    ...
    const result = await StaffModel.findById(id).populate('company').populate('project').lean();
  ...
  async function findAndPopulate({ email }) {
    await makeDb();
    const result = await StaffModel.findOne({ email }).populate('company').populate('project').lean();
    return result;
  }

  async function findAndUpdateArray(id, updateDetails) {
    await makeDb();
    const result = StaffModel.findByIdAndUpdate(id, { $push: updateDetails }).lean()
    return result;
  }
}